        ...
        if(legit)
        {
            if(version == "1.03-experimental")
            {
                ...
                if(line.find("A:") != std::string::npos)
                {
                    ...
                    vector<unsigned char> pxswap = Binary::file_to_uchar(handle.ReadToMemory(anim[2]+".px"));
                    int offset = 0;
                    int checks = 0;
                    ...
                    while(offset < length)
                    {
                        ...
                        else
                        {
                            ...
                            uint8_t r = Binary::get_uint8(pxswap, offset+4);
                            uint8_t g = Binary::get_uint8(pxswap, offset+5);
                            uint8_t b = Binary::get_uint8(pxswap, offset+6);
                            uint8_t a = Binary::get_uint8(pxswap, offset+7);

                            Object::Pixel px;
                            px.x = x;
                            px.y = y;
                            px.color = sf::Color(r,g,b,a);
...
void AnimatedObject::saveAnim()
{
    ofstream anim("data.anim", ios::binary);

    anim << "V4Mater-ver-1.03-experimental";
    anim << '\n';
    anim << "S:" << to_string_with_precision2(max_time-0.000001,6);
    anim << '\n';
    anim << "FR:" << to_string_with_precision2(animation_framerate,0);
    anim << '\n';
    anim << "XB:" << to_string_with_precision2(xBound,0);
    anim << '\n';
    anim << "YB:" << to_string_with_precision2(yBound,0);
    anim << '\n';

    cout << "animation names" << endl;

    float anim_time = 0;

    for(int a=0; a<animation_names.size(); a++)
    {
        anim << "A:" << to_string_with_precision2(anim_time,6) << ",";
        anim_time += getAnimationLengthByIndex(a);
        anim << to_string_with_precision2(anim_time,6) << "," << animation_names[a];
        if(animation_goto[a] != "")
        anim << "," << animation_goto[a];
        else
        anim << "," << "";
        anim << "," << to_string_with_precision2(animation_loop[a],0);
        anim << "\n";
    }

    cout << "animation bounds" << endl;

    for(int a=0; a<animation_bounds.size(); a++)
    {
        for(int b=0; b<animation_bounds[a].size(); b++)
        {
            anim << "AFB:" << to_string_with_precision2(a,0) << "," << to_string_with_precision2(b,0) << "," << to_string_with_precision2(animation_bounds[a][b].left,0) << "," << to_string_with_precision2(animation_bounds[a][b].top,0) << "," << to_string_with_precision2(animation_bounds[a][b].width,0) << "," << to_string_with_precision2(animation_bounds[a][b].height,0) << "," << to_string_with_precision2(animation_origins[a][b].x,0) << "," << to_string_with_precision2(animation_origins[a][b].y,0) << "\n";
        }
    }
    ...
    for(int i=0; i<objects.size(); i++)
    {
        ...
        {
            anim << "F:" << to_string_with_precision2(objects[i].frames[f].time, 6) << "," << i << "," << to_string_with_precision2(objects[i].frames[f].pos_x, 6) << "," << to_string_with_precision2(objects[i].frames[f].pos_y, 6) << "," << to_string_with_precision2(objects[i].frames[f].rotation, 6) << "," << to_string_with_precision2(objects[i].frames[f].or_x, 6) << "," << to_string_with_precision2(objects[i].frames[f].or_y, 6) << "," << to_string_with_precision2(objects[i].frames[f].scale_x, 6) << "," << to_string_with_precision2(objects[i].frames[f].scale_y, 6) << '\n';
        }
    }

    cout << "hitbox frames" << endl;

    for(int i=0; i<hitboxes.size(); i++)
    {
        for(int f=0; f<hitboxes[i].frames.size(); f++)
        {
            float x = hitboxes[i].frames[f].x - (hitboxes[i].frames[f].width/2);
            float y = hitboxes[i].frames[f].y - (hitboxes[i].frames[f].height/2);

            anim << "HBF:" << to_string_with_precision2(hitboxes[i].frames[f].time, 6) << "," << i << "," << to_string_with_precision2(hitboxes[i].frames[f].g_x, 6) << "," << to_string_with_precision2(hitboxes[i].frames[f].g_y, 6) << "," << to_string_with_precision2(x, 6) << "," << to_string_with_precision2(y, 6) << "," << to_string_with_precision2(hitboxes[i].frames[f].width, 6) << "," << to_string_with_precision2(hitboxes[i].frames[f].height, 6) << '\n';
        }
    }

    anim.close();

    cout << "Save PixelSwaps" << endl;

    for(int a=0; a<all_swaps.size(); a++)
    {
        ofstream pxswap(animation_names[a]+".px", ios::binary);

        for(int b=0; b<all_swaps[a].size(); b++)
        {
            uint32_t identifier = 0xAABBCCDD;
            uint16_t frame_id = b;
            uint8_t nullbyte = 0x0;

            pxswap.write((char*)&identifier, sizeof(uint32_t));
            pxswap.write((char*)&identifier, sizeof(uint32_t));

            for(int c=0; c<all_swaps[a][b].size(); c++)
            {
                Object::Pixel px = all_swaps[a][b][c];

                pxswap.write((char*)&px.x, sizeof(uint16_t));
                pxswap.write((char*)&px.y, sizeof(uint16_t));
                pxswap.write((char*)&px.color.r, sizeof(uint8_t));
                pxswap.write((char*)&px.color.g, sizeof(uint8_t));
                pxswap.write((char*)&px.color.b, sizeof(uint8_t));
                pxswap.write((char*)&px.color.a, sizeof(uint8_t));
            }
        }
...
int AnimatedObject::getSegmentIndexBasedOnTime(float time)
{
    for(int i=0; i<animation_begin.size(); i++)
    {
        if(time >= animation_begin[i])
        {
            if(time <= animation_end[i])
            {
                return i;
            }
        }
...
float AnimatedObject::getAnimationLengthByIndex(int index)
{
    float b,e;

    if(animation_begin.size() > 1)
    {
        b = animation_begin[index];
        e = animation_end[index];
    }
    else
    {
        b = animation_begin[0];
...
float AnimatedObject::getAnimationPosByIndex(int index)
{
    float e;
    ...
    return getAnimationLengthByIndex(index)-(e-cur_pos);
}
...
void AnimatedObject::SaveConfig()
{
    P4A handle;
    ...
    if(anim_path != "")
    {
        ...
        ///load whatever resources are used

        for(int i=0; i<animation_names.size(); i++)
        {
            handle.LoadFile(animation_names[i]+".px");
        }

        for(int i=0; i<animation_spritesheet.size(); i++)
        {
            animation_spritesheet[i].spritesheet.saveToFile(animation_names[i]+".png");
            handle.LoadFile(animation_names[i]+".png");
        }

        ///save p4a
        handle.SaveToFile(anim_path+"_new");
    }
}
...
void AnimatedObject::Draw(sf::RenderWindow& window)
{
    ...
    index = getSegmentIndexBasedOnTime(cur_pos);
    curFrame = floor(getAnimationPosByIndex(index) * animation_framerate);

    cout << "CurFrame: " << curFrame << " index: " << index << endl;

    float bound = floor(getAnimationLengthByIndex(index) * animation_framerate) - 1;
